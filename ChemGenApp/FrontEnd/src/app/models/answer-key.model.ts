...
export class AnswerKey {
	...
	public solubilityGeneration() {
		...
		solubility.push(solubilityData[cation1][anion1]);
		solubility.push(solubilityData[cation2][anion2]);
		solubility.push(solubilityData[cation1][anion2]);
		solubility.push(solubilityData[cation2][anion1]);
	...
	public molecularNotationGeneration() {
		...
		ion_charge.push(charge_table[cation1]);
		ion_charge.push(charge_table[anion1]);
		ion_charge.push(charge_table[cation2]);
		ion_charge.push(charge_table[anion2]);
		...
		reactant1 = (cation1 + chargeTemp[1] + anion1 + chargeTemp[0]);
		reactant2 = (cation2 + chargeTemp[3] + anion2 + chargeTemp[2]);
		product1 = (cation1 + chargeTemp[3] + anion2 + chargeTemp[0]);
		product2 = (cation2 + chargeTemp[1] + anion1 + chargeTemp[2]);
	...
	public stoichiometryGeneration() {
		...
		var f = molecularNotationGeneration();
		...
		var atmoicWeights = JSON.parse(txt);
		
		// A list of the weights of the cations and anions
		var listOfWeights = [];
		
		// load the list with the appropriate values
		listOfWeights.push(atomicWeights[cation1]);
		listOfWeights.push(atomicWeights[anion1]);
		listOfWeights.push(atomicWeights[cation2]);
		listOfWeights.push(atomicWeights[anion2]);
		
		/** NOTE: This is a reference for filling the strings below
		f[1] is reactant molecule #1 (cation1 + anion1 with subscript stuff)
		f[3] is reactant molecule #2 (cation2 + anion2 with subscript stuff)
		f[5] is product molecule #1 (cation1 + anion2 with subscript stuff)
		f[7] is product molecule #2 (cation2 + anion1 with subscript stuff)
		*/
		...
		*/
		...
		var mol_compound_1 = grams1/(listOfWeights[0]*(f[9]) + listOfWeights[1]*(f[8]));
		var mol_compound_2 = grams2/(listOfWeights[3]*(f[10]) + listOfWeights[2]*(f[11]))
		...
		*/
		...
		if (P1_mols_R1 < P1_mols_R2 && P2_mols_R1 < P2_mols_R2) {
			...
			var excess_grams = grams2 - gramsUsed;
			...
			*/
			...
			var excess_grams = grams1 - grams_used;
			var sum_of_masses = (excess_grams + Yield_P1 + Yield_P2);
